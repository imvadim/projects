...
public class GmailTestFactoryMethod extends BaseTest {
    ...
    private void loginToGmailOk() {
        Assert.assertTrue(new SignInPageOldActions(getDriver()).loginToGmail(username, password).loginIsCorrect(title),
...
//    , dependsOnGroups = "checkpointNotInDraftsOK"
    ...
    private void EmailInDraftsOK() {
        ...
        Assert.assertTrue(new InboxPageOldActions(getDriver()).fillLetter(email).toDrafts().inDrafts(email.getSubject()));
    }
    ...
    public void notInDraftsOK() {
        Assert.assertFalse(new InboxPageOldActions(getDriver()).sendEmail().toDrafts().inDrafts(subject));
    }
    ...
    private void inSentOK() {
        Assert.assertTrue(new InboxPageOldActions(getDriver()).toSent().inSent(subject));
